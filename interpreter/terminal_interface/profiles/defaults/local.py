...
def download_model(models_dir, models, interpreter):
    ...
    free_disk_space = psutil.disk_usage("/").free / (1024 * 1024 * 1024
                                                     )  # Convert bytes to GB
    ...
    try:
        model_list = [
            ...
            },
            {
                ...
                5.15,
                "url":
                "https://huggingface.co/jartine/Mistral-7B-Instruct-v0.2-llamafile/resolve/main/mistral-7b-instruct-v0.2.Q5_K_M.llamafile?download=true",
            },
            {
                "name":
                ...
                7.33,
                "url":
                "https://huggingface.co/jartine/wizardcoder-13b-python/resolve/main/wizardcoder-python-13b.llamafile?download=true",
            },
            {
                "name":
                ...
                22.23,
                "url":
                "https://huggingface.co/jartine/WizardCoder-Python-34B-V1.0-llamafile/resolve/main/wizardcoder-python-34b-v1.0.Q5_K_M.llamafile?download=true",
            },
            {
                "name":
                ...
                30.03,
                "url":
                "https://huggingface.co/jartine/Mixtral-8x7B-Instruct-v0.1-llamafile/resolve/main/mixtral-8x7b-instruct-v0.1.Q5_K_M.llamafile?download=true",
            },
        ]

        # Filter models based on available disk space and RAM
        filtered_models = [
            model for model in model_list if model["size"] <= free_disk_space
            and model["file_name"] not in models
        ]
        if filtered_models:
            time.sleep(1)
...
""")
    ...
    try:
        ...
        names = [
            line.split()[0].replace(":latest", "") for line in lines[1:]
            if line.strip()
...
""")
    ...
    jan_model_name = model_name_answer[
        "jan_model_name"] if model_name_answer else None
    ...
    if not models:
        ...
        questions = [
            inquirer.List(
                "model",
                message=
                "Select a Llamafile model to run or download a new one:",
                choices=model_choices,
            )
        ]
        answers = inquirer.prompt(questions)
