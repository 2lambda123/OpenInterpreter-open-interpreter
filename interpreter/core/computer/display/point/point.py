...
def get_element_boxes(image_data, debug):
    ...
    if os.getenv("OI_POINT_PERMUTATE", "False") == "True":
        ...
        for _ in range(10):
            ...
            random_block_size = secrets.SystemRandom().choice(range(1, 11, 2)
            )  # Random block size in range 1 to 10, but only odd numbers
            random_block_size = 11
            random_adaptive_method = secrets.SystemRandom().choice([cv2.ADAPTIVE_THRESH_MEAN_C, cv2.ADAPTIVE_THRESH_GAUSSIAN_C]
            )  # Random adaptive method
            random_threshold_type = secrets.SystemRandom().choice([cv2.THRESH_BINARY, cv2.THRESH_BINARY_INV]
